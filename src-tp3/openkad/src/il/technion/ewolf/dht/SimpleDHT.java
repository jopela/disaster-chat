...
/**
 * Implements a DHT in the most strait forward way. put finds the nodes closest
 * to the key using a key based routing and sends store message to all the found
 * nodes. get finds the nodes closest to the key and sends them a get request.
 * 
...
public class SimpleDHT implements DHT {
	...
	FindvalueHandler findvalueHandler, StoreMessageHandler storeMessageHandler,
	...
	public SimpleDHT create() {
		...
		findvalueHandler.setStorage(storage).register(dhtName, kbr);

		storeMessageHandler.setStorage(storage).register(dhtName, kbr);
	...
	public void put(Key key, Serializable data) {
		putOperationProvider.get().setKey(key).setData(data)
				.setDhtName(dhtName).setStorage(storage)
				.setAge(System.currentTimeMillis()).doPut();
	}
	...
	public List<Serializable> get(Key key) {
		return getOperationProvider.get().setKey(key).setDhtName(dhtName)
				.setStorage(storage).doGet();
	}
	...
	public static void main(String[] args) throws Exception {
		...
		injector.getInstance(DHT.class).setName("dht").setStorage(storage)
				.create();
	}
