...
/**
 * Does the complex get operation: sends a findValueRequest to all the closest
 * nodes to the key according to the key based routing network and waits for
 * responses. Once all responses have recved, merge them into a list
 * 
...
public class GetOperation {
	...
	/**
	 * Does the operation. You must set the storage, key and dhtName before
	 * calling this method
	 * 
	...
	public List<Serializable> doGet() {
		if (dhtName == null || storage == null || key == null)
			throw new IllegalArgumentException(
					"missing param in dht get operation");
		...
		final Set<Serializable> results = Collections
				.synchronizedSet(new HashSet<Serializable>());
		final CountDownLatch latch = new CountDownLatch(nodes.size());

		for (Node n : nodes) {
			kbr.sendRequest(n, dhtName + findValueRequestHandlerName,
					findValueRequest, null,
					new CompletionHandler<Serializable, Void>() {
