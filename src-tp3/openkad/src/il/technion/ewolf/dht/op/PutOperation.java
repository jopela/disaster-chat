...
/**
 * Does the put operation: send storeMessage to all closest nodes according to
...
public class PutOperation {
	...
	@Inject
	PutOperation(KeybasedRouting kbr,
	...
	public void doPut() {
		if (data == null || key == null || dhtName == null || age == 0
				|| storage == null)
			throw new IllegalArgumentException(
					"missing params in dht put operation");

		storage.store(key, age, data);

		List<Node> nodes = kbr.findNode(key);

		StoreMessage storeMessage = storeMessageProvider.get().setContent(data)
				.setAge(age).setKey(key);

		for (Node n : nodes) {
			try {
				kbr.sendMessage(n, dhtName + storeMessageHandlerName,
						storeMessage);
			} catch (Exception e) {
				e.printStackTrace();
			}
