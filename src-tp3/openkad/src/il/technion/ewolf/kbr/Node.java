...
/**
 * Represents contact information about a node in the network. This class is
 * serializable and can be sent/saved for other/later use by any KeybasedRouting
 * local or remote.
 * 
...
public class Node implements Serializable, Comparable<Node> {
	...
	/**
	 * The endpoints map is a map containing all the nodes available protocols
	 * as keys and the protocol's port as values
	 * 
	 ...
	 * 
	 * @param scheme
	 *            the protocol name (such as http or openkad.udp)
	 * @return the uri
	 */
	public URI getURI(String scheme) {
		try {
			return new URI(scheme + "://" + addr.getHostAddress() + ":"
					+ getPort(scheme) + "/" + key.toBase64());
	...
	/**
	 * Append a new protocol for this node. Use this method to add your own
	 * protocol support (such as http support). Make sure you add the new
	 * protocol BEFORE joining a network
	 * 
	 * @param scheme
	 *            the protocol name
	 * @param port
	...
	/**
	 * Get the port number of a given protocol name. May throw a
	 * NullPointerException if protocol was not added
	 * 
	 * @param scheme
	 *            the protocol name
