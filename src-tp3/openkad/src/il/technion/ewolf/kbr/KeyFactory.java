...
public interface KeyFactory {
	 ...
	 * 
	 * @param pow2Max
	 *            power 2 of the max integer value
	 * @return a new random key where its integer value is between 0 and
	 *         2^pow2Max - 1
	 */
	public Key generate(int pow2Max);

	/**
	 * Convert a given byte array to a key. May throw an
	 * IllegalArgumentException if the number of bytes is incorrect.
	 * 
	 * @param bytes
	 *            the key's bytes
	...
	/**
	 * Create a new key from an array of topics. If 2 arrays of topics are
	 * equals then the 2 keys generated from them will be equal If 2 arrays of
	 * topics are not equals the chances of the keys to be equal are next to
	 * none
	 * 
	 * @param topics
