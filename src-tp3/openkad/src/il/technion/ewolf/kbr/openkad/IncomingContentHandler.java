 ...
 * 
 * @param <A>
 *            any arbitrary attachment
 */
public class IncomingContentHandler<A> implements
		CompletionHandler<KadMessage, A> {
	 ...
	 *            the executor for all incoming requests that should be handled
	 *            by the KeybasedRouting user
	 * @param localNode
	 *            the local node
	 * @param kadServer
	 *            the KadServer used to send messages
	 */
	@Inject
	IncomingContentHandler(
			@Named("openkad.executors.client") ExecutorService clientExecutor,
			@Named("openkad.local.node") Node localNode, Communicator kadServer) {
	...
	private void handleRequest(final ContentRequest req) {
		...
		clientExecutor.execute(new Runnable() {
			@Override
			public void run() {
				Serializable resContent = handler.onIncomingRequest(
						req.getSrc(), tag, req.getContent());
				try {
					kadServer.send(req.getSrc(), req
							.generateResponse(localNode).setContent(resContent));
				} catch (IOException e) {
