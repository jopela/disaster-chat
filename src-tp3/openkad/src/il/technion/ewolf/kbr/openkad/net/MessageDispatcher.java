...
/**
 * Handle all the messages different states. A request state: init -> sent ->
 * response received -> callback invoked
 * 
 * A message state: init -> expecting -> message received -> callback invoked ->
 * back to expecting or end
 * 
...
public class MessageDispatcher<A> {
	...
	private final Set<MessageDispatcher<?>> nonConsumableexpecters; // must be
																	// sync'ed
																	// set
	...
	public MessageDispatcher<A> setCallback(A attachment,
			CompletionHandler<KadMessage, A> callback) {
	...
	public void send(Node to, KadRequest req) {
		...
		try {
			/*
			 * if (!outstandingRequests.offer(this, timeout,
			 * TimeUnit.MILLISECONDS)) throw new RejectedExecutionException();
			 */
