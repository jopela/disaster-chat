 ...
 * 
 */
public class JsonKadSerializer extends KadSerializer implements
		JsonSerializer<Serializable>, JsonDeserializer<Serializable> {

	private final Gson gson;

	private static final String classPackage = KadMessage.class.getPackage()
			.getName() + ".";
	...
	public KadMessage read(InputStream in) throws IOException,
		...
		try {
			...
			msg = gson
					.fromJson(reader, Class.forName(classPackage + clazzName));
			reader.endArray();
	...
	public Serializable deserialize(JsonElement json, Type typeOfT,
		...
		byte[] src = Base64.decodeBase64(json.getAsJsonPrimitive()
				.getAsString());
		try {
