...
public class JoinOperation {
	...
	@Inject
	JoinOperation(Provider<FindNodeOperation> findNodeOperationProvider,
			Provider<PingRequest> pingRequestProvider,
			Provider<MessageDispatcher<Void>> msgDispatcherProvider,
			Provider<KadNode> kadNodeProvider, KBuckets kBuckets,
	 ...
	 * 
	 * @see
	 * il.technion.ewolf.kbr.openkad.op.JoinOperation#addBootstrap(java.util
	 * .Collection)
	 */
	...
	public void doJoin() {
		...
		CompletionHandler<KadMessage, Void> callback = new CompletionHandler<KadMessage, Void>() {
			...
			public void completed(KadMessage msg, Void nothing) {
				try {
					kBuckets.insert(kadNodeProvider.get().setNode(msg.getSrc())
							.setNodeWasContacted());
				} finally {
					latch.countDown();
				}
		...
		findNodeOperationProvider.get().setKey(localNode.getKey()).doFindNode();
		...
		try {
			// timer.scheduleAtFixedRate(refreshTask, refreshInterval,
			// refreshInterval);
