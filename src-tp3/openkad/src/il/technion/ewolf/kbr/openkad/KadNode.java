...
/**
 * Wrapper for Node type. Used to keep track on the following: 1. last contact
 * time - the most recent time a message was received from this node 2. being
 * ping - expecting a ping to be received from this node
 * 
...
public class KadNode {
	...
	/**
	 * Sets the contained node. Do not re-set the node after you start using
	 * this class
	 * 
	 * @param node
	 *            the wrapped node
	...
	/**
	 * Signals this node is being pinged. Important: If true is returned, you
	 * must call releasePingLock to allow this node to be pinged in the future
	 * 
	 * @return true if the node was not being pinged, false if it was already
	 *         locked
	 */
	...
	/**
	 * Get the last time a message was received from this node (the last time
	 * setNodeWasContacted was invoked)
	 * 
	...
	/**
	 * Check if the last message from this node was received less than the given
	 * time span
	 * 
	 * @param validTimespan
	 *            maximum time span in millies
	 * @return true if setNodeWasContacted was invoked in the passed
	 *         validTimespan millies
	 */
	public boolean isPingStillValid(long validTimespan) {
		return lastContactTimestamp.get() + validTimespan > System
				.currentTimeMillis();
	}
	...
	/**
	 * Explicitly sets the last time a message was received from the wrapping
	 * node
	 * 
	 * @param lastContact
	 */
	...
	/**
	 * Sets the last contact time to 0, which will cause hasNeverContacted to
	 * return true
	 */
