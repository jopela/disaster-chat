...
/**
 * Find value operation according to the colors algorithm TODO: add a link to
 * the published article
 * 
 * @author eyal.kibbar@gmail.com
 * 
 */
public class ColorFindValueOperation extends FindValueOperation implements
	...
	private synchronized Node takeColorUnqueried() {
		...
		if (allUnqueried.size() > 1) {
			allUnqueried = sort(allUnqueried, on(Node.class).getKey(),
					colorComparator);
		}
		$ = allUnqueried.get(0);
	...
	public List<Node> doFindValue() {
		...
		List<Node> colorClosest = kBuckets.getClosestNodesByKey(key,
				nrCandidates);
		...
		do {
			...
			Node n = ((++i % concurrency) == 0) ? takeColorUnqueried()
					: takeUnqueried();
